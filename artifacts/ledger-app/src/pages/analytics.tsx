...
export default function AnalyticsPage({ selectedMonth }: { selectedMonth: string }) {
  ...
  const monthlyDataAll = useMemo(() => {
    ...
    return sortedMonths.map((month) => {
      ...
      const scoped = filterTransactionsToCalendarMonth(txs, month);
      const audited = filterAuditedTransactions(scoped);
      const { income: totalIncome, spending: totalSpending } = computeAuditedMonthTotals(scoped);
      const catTotals = computeAuditedCategoryTotals(scoped);
      const incTotals = computeIncomeTotals(audited);
  ...
  const currentMonthData = useMemo(() => {
    ...
    const scoped = filterTransactionsToCalendarMonth(txs, monthKey);
    const audited = filterAuditedTransactions(scoped);
    const { income: totalIncome, spending: totalSpending } = computeAuditedMonthTotals(scoped);
    const catTotals = computeAuditedCategoryTotals(scoped);
    const incTotals = computeIncomeTotals(audited);
  ...
  const monthlyBillsTotal = computeBillManagerMonthTotals(
    bills || [],
    monthKey,
    (allTxs || []).filter((t) => t.month === monthKey),
  ).totalAmount;
